...
th_gt = me.params(ground,N)

ll_gt = th_gt.loglike_w(w_all,T_all)


beta,gamma = me.naive_estimation(T_all,w_all)

theta = np.array((beta,gamma))
th_me = me.params(theta,N)

th = th_me
ll = th.loglike_w(w_all,T_all)

me.update_th(ll,w_all,T_all,th,S_prop)
...
for i in range(len(ll_list),N_sam):
    ll,th  = me.update_th(ll,w_all,T_all,th,S_prop)    
...
try:
    df2 = pd.read_csv('times/ME_times_20.csv')
except:
    df2 = pd.DataFrame()
label = 'N={}'.format(N)
df2[label] = np.stack(times100)
df2.to_csv('times/ME_times_20.csv',index=False)
